...
public class LookMinigameHandler : MonoBehaviour
{
    ...
    private bool closeEnoughToStart = true;
    ...
    void Update()
    {
        ...
        if (started)
        {
            ...
            if (!movingLeft && !movingRight)
            {
                ...
                if (rightPressed)
                {
                    if (Mathf.Sign(pathObjects[tempIndexRight].position.x - pathObjects[tempIndexLeft].position.x) == Mathf.Sign(Camera.main.transform.right.x))
                    {
