...
public class ChoiceScreen : MonoBehaviour
{
    ...
    public static IEnumerator ShowingChoices(params string[] choices)
    {
        ...
        {
            Debug.Log("WAITING FOR HEADER TO REVEAL");
            yield return new WaitForEndOfFrame();
        }
    ...
    private static void SetLayoutSpacing()
    {
        int i = choices.Count;
        if (i < 3)
        {
    ...
    private static void CreateChoice(string choice)
    {
        GameObject choiceObj = Instantiate(instance.choicePrefab.gameObject, instance.choicePrefab.transform.parent);
        choiceObj.SetActive(true);

        ChoiceButton b = choiceObj.GetComponent<ChoiceButton>();
    ...
    {
        if (choices.Count > choiceIndex && choiceIndex >= 0)
        {
